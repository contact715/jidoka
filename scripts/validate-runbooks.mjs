...
// ── CLI args ───────────────────────────────────────────────────────────────
// Разбор строгий (2026-09-16): незнакомый флаг — код 2 до чтения runbook-ов и до правки
// _INDEX.md (раньше опечатка `--dyr --emit-index` молча переписывала индекс).
export const CLI = {
  name: 'validate-runbooks',
  summary: 'Инварианты V1–V7 для docs/runbooks/*.md и _INDEX.md; код 1 при нарушении.',
  options: {
    dry: { type: 'boolean', desc: 'только проверка, без записи файлов' },
    'emit-index': { type: 'boolean', desc: 'обновить отметку «Last validated» в _INDEX.md' },
  },
};
...
function emitIndexSummary(runbookFiles, isDry = false) {
...
function main(values = {}) {
  ...
  const emitIndex = values['emit-index'] === true;
  const violations = [];
  const warnings = [];
  ...
  // ── --emit-index ───────────────────────────────────────────────────────
  if (emitIndex) {
    emitIndexSummary(runbookFiles, isDry);
  }
}
