...
function selfTest() {
  ...
  /*
    ФОРМАТ СВОДКИ ОБЩИЙ С ОСТАЛЬНЫМИ, а не свой. Здесь стояло «пройдено N,
    провалено M» — слово, потом число. Прибор selftest-reality читает сводку
    как «число, потом слово» (`${ran} прошло, 0 упало`), и так печатают
    двадцать скриптов из двадцати одного.

    Из-за расхождения этот скрипт числился слепым пятном: выходил с нулём,
    прибор не находил в выводе ни сводной пары, ни маркеров, и объявлял, что
    самопроверка не запускалась. На деле она запускалась и давала 39 проверок.

    Чинить надо было именно здесь, а не расширять прибор: один скрипт против
    двадцати — это отклонение, а не второй законный формат. Прибор, который
    понимает два способа сказать одно и то же, со временем понимает пять.
  */
  console.log(`\n[самопроверка] ${ok} прошло, ${bad} упало`);
  return bad === 0 ? 0 : 1;
}
