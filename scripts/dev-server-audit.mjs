...
/** Тревожное ли состояние. Пороги абсолютные; нагрузка соотнесена с ядрами. */
export function isStarving(m) {
  if (!m) return false;
  return m.swapUsedGb >= 8 || m.load >= m.cores * 1.5;
}
...
function main(argv) {
  ...
  const m = memory();
  if (m) {
    const alarm = isStarving(m);
    console.log(`[память] в подкачке ${m.swapUsedGb} ГБ, сжато ${m.compressorGb} ГБ, нагрузка ${m.load} на ${m.cores} ядер${alarm ? '  ← машина задыхается' : ''}`);
  }
...
function selfTest() {
  ...
  t('ужатый файл подкачки не маскирует беду', isStarving({ swapUsedGb: 10.2, load: 3, cores: 12 }));
  t('и не поднимает тревогу там, где её нет', !isStarving({ swapUsedGb: 1.1, load: 3, cores: 12 }));
  ...
  t('подкачка 10 ГБ — тревога', isStarving({ swapUsedGb: 10, load: 1, cores: 8 }));
  t('подкачка 2 ГБ — спокойно', !isStarving({ swapUsedGb: 2, load: 1, cores: 8 }));
  t('нагрузка 20 на 8 ядер — тревога', isStarving({ swapUsedGb: 0, load: 20, cores: 8 }));
  t('нагрузка 6 на 8 ядер — спокойно', !isStarving({ swapUsedGb: 0, load: 6, cores: 8 }));
  t('нет замера — не тревожим зря', !isStarving(null));
  t('процент больше не решает: ужатый файл не даёт тревоги',
    !isStarving({ swapUsedGb: 2, load: 3, cores: 8 }));
