...
export function extractEntities(text) {
  ...
  for (const line of lines) {
    ...
    // Границы юникодные, НЕ \b: в JavaScript \b определена через [A-Za-z0-9_], поэтому между
    // пробелом и русской заглавной границы нет и правило не находило НИ ОДНОГО имени.
    // Замер 2026-08-26 на строке «Владелец Дмитрий обсуждал Ребрендинг с Катей»: было 0
    // совпадений, стало 3. Нашёл scripts/cyrillic-boundary.mjs.
    for (const m of line.matchAll(/(?<![.!?]\s)(?<!^)(?<![\p{L}\p{N}])[А-ЯЁ][а-яё]{3,}(?![\p{L}\p{N}])/gmu)) {
      add(m[0], "имя", line);
    }
  }
