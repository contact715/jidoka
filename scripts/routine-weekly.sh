...
## 4b. Застрявшие рекомендации без третьего исхода

Запись старше двух недель обязана иметь ЛИБО исполнителя в очереди, ЛИБО датированное
отклонение с причиной. «Висит дальше» умолчанием больше не является. Отклонить это
законный исход; незаконно только молчание.

EOF

if [ -f scripts/kaizen-dispatch.mjs ]; then
  # Отчётный прогон: рутина СОБИРАЕТ картину, а зубы (--gate) принадлежат недельному
  # прогону кайдзена ПЕРЕД выпуском нового плана. Ронять еженедельную сводку на долге
  # значило бы научить её пропускать.
  node scripts/kaizen-dispatch.mjs stalled >> "$REPORT" 2>&1 || true
else
  echo "(scripts/kaizen-dispatch.mjs not present; skipping)" >> "$REPORT"
fi
