...
1. **Сначала бизнес-вопросы.** Пойми ЧТО и ЗАЧЕМ до того как КАК. Через AskUserQuestion
   уточни бизнес-логику, пользователей, ограничения, критерии успеха. НЕ начинай код без
   ясности. Это самый важный шаг — здесь обсуждается бизнес-логика.

2. **Мастер-спека.** Дispatch архитекторов ПАРАЛЛЕЛЬНО (Task tool):
   micro-architect (взгляд изнутри), macro-architect (конкуренты/рынок),
   surface-cartographer («это уже где-то есть?»), design-system-architect (дизайн-контракт).
   Затем chief-architect (тимлид спеки) синтезирует мастер-спеку из их брифов. Дальше —
   модульные спеки по необходимости.
