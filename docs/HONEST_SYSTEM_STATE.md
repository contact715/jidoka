...
- **OS-sandbox: три слоя, kernel-уровень есть на macOS.** `policy-sandbox` (policy-proxy) +
  `policy-enforce-hook` (real-time блок записи в L0/security пути через PreToolUse, exit 2) +
  `sandbox-run` — РЕАЛЬНАЯ kernel-изоляция через `sandbox-exec`/Seatbelt: команда читает всё, но
  пишет ТОЛЬКО в declared scope, сеть запрещена, enforced ядром. Доказано `--verify`: out-of-scope
  запись → «Operation not permitted» от ОС, сеть → exit 6. Честная граница: kernel-sandbox = macOS
  (sandbox-exec); на Linux `sandbox-run` деградирует до firejail/bwrap или честный skip;
  контейнер-на-субагента (Docker) — отдельный слой, не реализован.
