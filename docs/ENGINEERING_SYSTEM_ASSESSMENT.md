...
- **Per-wave** — skill-extractor пишет навык, если паттерн повторился (так накоплены 33 навыка; аналог skill-library из Voyager).
...
97 npm-команд автоматизируют весь цикл — от `wave:status` до `compute:dora` и `detect:injection`.
